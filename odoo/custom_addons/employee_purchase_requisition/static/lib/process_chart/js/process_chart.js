(function($) {
    $.fn.processChart = function(options) {
        var opts = $.extend({}, $.fn.processChart.defaults, options);
        return new ProcessChart($(this), opts);        
    }

    $.fn.processChart.defaults = {
    ...
    function ProcessChart($container, opts){
        var data = opts.data;
        var nodes = {};
        var rootNodes = [];
        this.opts = opts;
        ...
        this.draw = function(){
            ...
            $container.find('.bkg-process-node').click(function(){
            ...
            if(opts.allowEdit){
                $container.find('.bkg-process-node h2').click(function(e){
            ...
            $container.find('.process-add-button').click(function(e){
                var thisId = $(this).parent().attr('node-id');
            ...
            $container.find('.process-del-button').click(function(e){
                var thisId = $(this).parent().attr('node-id');
        ...
        this.startEdit = function(id){
            var inputElement = $('<input class="process-input" type="text" value="'+nodes[id].data.name+'"/>');
        ...
        // draw process chart
        $container.addClass('processChart');
        self.draw();
    }
        ...
        this.formatNode = function(opts){
            ...
            if(typeof data.name !== 'undefined'){
                if (self.data.status == true){
                    nameString = "<h2 class='bkg-h2-process bkg-done-status' type='" + self.data.type +"' type_id='" + self.data.type_id +"'>"+self.data.name+'</h2>';
                } else if (self.data.status == false){
                    nameString = "<h2 class='bkg-h2-process bkg-not-done-status' type='" + self.data.type +"' type_id='" + self.data.type_id +"'>"+self.data.name+'</h2>';
                } else {
            ...
            if(opts.showControls){
                var buttonsHtml = "<div class='process-add-button'>"+opts.newNodeText+"</div><div class='process-del-button'></div>";
            }
            else{
                buttonsHtml = '';
            }
            return "<div class='bkg-process-node' node-id='"+this.data.id+"'>"+nameString+descString+buttonsHtml+"</div>";
        }
    }
